...
(function () {
  ...
  function appendGridItems(items) {
    ...
    items.forEach(function (item) {
      grid.appendChild(
        buildGridItem(item.yourName, item.saySomethingNiceAboutBolu)
      );
    });
  }
  ...
  function resizeGridItem(item) {
    ...
    var rowSpan = Math.ceil(
      (item.querySelector(".wish").getBoundingClientRect().height + rowGap) /
        (rowHeight + rowGap)
    );
  ...
  function fetchData() {
    ...
    xmlhttp.onreadystatechange = function () {
      if (this.readyState == 4 && this.status == 200) {
        ...
        //   "wishes-for-bolu-2",
        //   JSON.stringify(data.formResponses1)
        // );
        ...
        hideLoader();
        // var wishes = localStorage.getItem("wishes-for-bolu-2");
        var wishes = false;
        if (wishes) {
          ...
          var gridContainer =
            document.getElementsByClassName("wishes-container")[0];
    ...
    xmlhttp.open(
      "GET",
      "https://api.sheety.co/646c88ae6950c8a1a26087579f5b3e6f/bbd/formResponses1",
      true
    );
    xmlhttp.send();
  }
